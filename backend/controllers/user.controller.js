  ...
  try {
    ...
    if (!suggestedUsers || suggestedUsers.length === 0) {
      return res
        .status(404)
        .json({
          message: "Currently don not have any suggested users",
          success: false,
        });
    }
    ...
    return res
      .status(200)
      .json({ suggestedUsers, success: true, users: suggestedUsers });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: "Server error", success: false });
  }
...
export const followOrUnfollow = async (req, res) => {
  try {
    ...
    if (userId === targetUserId) {
      return res
        .status(400)
        .json({ message: "Cannot follow/unfollow yourself", success: false });
    }
